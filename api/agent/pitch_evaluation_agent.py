...
def run_evaluation(transcript: str, panel_count: int, vector_db) -> List[str]:
    ...
    for i in range(panel_count):
        ...
        for chunk in chunks:
            docs = vector_db.similarity_search(chunk, k=3)
...
def evaluate_pitch_audio(file: UploadFile, user_panel_count: int) -> PitchEvaluateResponse:
    ...
    panel_feedback = run_evaluation(transcript, user_panel_count, vector_db)
    return PitchEvaluateResponse(transcript=transcript, panel_feedback=panel_feedback)
